...
bool MacUtils::registerGlobalShortcut(const QString& name, Qt::Key key, Qt::KeyboardModifiers modifiers)
{
    ...
    gs->hotkeyId = {"kpxc", m_nextShortcutId};
    ...
    if (::RegisterEventHotKey(gs->nativeKeyCode, gs->nativeModifiers, gs->hotkeyId, GetApplicationEventTarget(), 0, &gs->hotkeyRef) != noErr) {
        return false;
    }
